...
class OAuthAuthentication(object):
    ...
    def is_authenticated(self, request):
        ...
        if self.is_valid_request(request):
            ...
            token_val = request.POST.get("access_token", False)
            if not token_val:
                try:
                    body_json = json.loads(request.body)
                    if "access_token" in body_json:
                        token_val = body_json["access_token"]
                    if "oauth_token" in body_json:
                        token_val = body_json["oauth_token"]
                except:
                    pass  # no good, likely not json body

            try:
                token = Token.objects.get(key=token_val, token_type=2)
                request.user = token.user
                return True
            ...
            # A bearer token
            try:
                token = Token.objects.get(key=request.GET.get("access_token"), token_type=2)
                request.user = token.user
                return True
            except Token.DoesNotExist:                
                pass # invalid token
