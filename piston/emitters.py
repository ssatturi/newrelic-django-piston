...
class Emitter(object):
    ...
    def construct(self):
        ...
        def _model(data, fields=()):
            ...
            """
            ret = { }
            handler = self.get_typemapper(data, self.anonymous)
            ...
            if handler or fields:
                ...
                if not fields:
                    ...
                    # mapped = self.get_typemapper(data, self.anonymous)
                    mapped = handler
                ...
                for maybe_field in get_fields:
                    if isinstance(maybe_field, (list, tuple)):
                        ...
                        if maybe is not None:
                            if callable(maybe) and not hasattr(maybe, 'all'):
            ...
            if self.get_typemapper(data, self.anonymous):
                handler = self.get_typemapper(data, self.anonymous)
    ...
    def get_typemapper(self, model, anonymous):
        if hasattr(model, "apiMapper"):
            return model.apiMapper
        return self.in_typemapper(type(model), self.anonymous)

    def in_typemapper(self, model, anonymous):        
        types = list( self.typemapper.iteritems() )
        for klass, (km, is_anon) in types:
...
class JSONEmitter(Emitter):
    ...
    def render(self, request):
        seria = json.dumps(self.construct(), default=json_default, ensure_ascii=False)
        return seria

    def stream_render(self, request, stream=True):
        """Returns an iterator if self.data is a QuerySet.  The iterator represents a JSON
           encoded list with each query-row in a single yield """
        if not stream:
            yield self.render(request)
            return

        if not isinstance(self.data, QuerySet):
            yield self.render(request)
            return
        ...
        for row in self.data:
            rowString = json.dumps(self.construct_one(row), default=json_default, ensure_ascii=False)
            if hadOne:
                yield u', ' + rowString
            else:
                yield rowString
...
class JSONPEmitter(Emitter):
    ...
    def render(self, request):
        ...
        seria = json.dumps(self.construct(), default=json_default, ensure_ascii=False, indent=4)
