...
from django.http import (HttpResponse, Http404, HttpResponseNotAllowed, StreamingHttpResponse,
...
class Resource(object):
    ...
    def __call__(self, request, *args, **kwargs):
        ...
        # content
        if isinstance(response.data, HttpResponse) and \
                hasattr(response.data, '_base_content_is_iter') and \
                response.data._base_content_is_iter:
        ...
        try:
            ...
            """
            if handler and handler.ALLOW_STREAM and request.method == 'GET' and self.stream and isinstance(srl.data, QuerySet):
            ...
            if not isinstance(stream, HttpResponse):
                ...
                resp = StreamingHttpResponse(stream, content_type=ct, status=status_code)
    ...
    def email_exception(self, reporter, exception, request):
        try:
            # Logging to rollbar instead of email now
            rollbar.init(settings.ROLLBAR['access_token'], environment=settings.ROLLBAR['environment'])
            rollbar.report_exc_info(sys.exc_info(), request, {}, {'level': 'error'})
        except:
            # Fail Silently
            pass
    ...
    def error_handler(self, response, e, request, meth):
        ...
        elif isinstance(e, (PistonException, PistonBadRequestException, PistonForbiddenException, PistonMethodException, PistonNotFoundException, PistonUnauthorizedException)):
        ...
        # TODO: As we start using Piston exceptions, the following 2 errors can be phased out
            ...
            if hasattr(e,"message_dict"):
                response.error_message = "\n".join("%s: %s" % (k[0],k[1][0]) for k in e.message_dict.iteritems())
            else:
